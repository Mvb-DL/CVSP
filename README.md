...
- `imgsz`: 640 → **800**# CHANGELOG — SAR Person Detection (YOLOv8)

> Lückenlose, rein faktische Verlaufs- und Änderungsdokumentation aller Trainings- und Validierungsläufe (ohne Begründungen), inkl. Datensätze, Parameterstände und Ergebnisse.  
> Zeitraum: Nov 2025
...
**Modell & Setup**  
- Backbone: YOLOv8n → **YOLOv8m (25M Params)**  
- `imgsz`: 640 → **800**  
- Optimizer: SGD → **AdamW**  
- Epochen: 5 → **50** → **70**  
- Evaluation/IoU: **0.5**  
- Starke Augmentierung:  
  `hsv_h=0.04, hsv_s=0.9, hsv_v=0.7, degrees=15, translate=0.3, perspective=0.002, mosaic=1.0, mixup=0.2, copy_paste=0.3`
...
- Log-Eintrag „yolo11n.pt“ stammt aus AMP/Font-Check; **Train-Backbone bleibt v8m**.  
- Einmalige Warnung „NMS time limit exceeded“ in einem frühen Val-Durchlauf.
...
**Startgewichte**  
- `best` aus Stage-1 (70-Epochen-Run)

**Trainingsparameter**  
- Epochen: **12**  
- `lr0=0.005`, `lrf=0.01`, `cos_lr=True`, `warmup_epochs=1`, `weight_decay=5e-4`  
- Abgeschwächte Augmentierung:  
  `hsv_h=0.02, hsv_s=0.8, hsv_v=0.55, degrees=10, mosaic=0.0, mixup=0.0, copy_paste=0.0, close_mosaic=1`  
...
**Quellen & Zielstruktur (unter D:\data)**  
...
- **NTUT4K** (überwiegend **negativ**, keine Labels) → **D:\data\NTUT4KYOLO** → YAML: `cfg/ntut4kyolo.yaml`  
- **Zenodo 7740081** → keine Bilder auffindbar
...
- Filtert ausschließlich **person** (Klasse 0); negative Bilder → leere Labeldateien  
- Kombinierte YAMLs:  
  - `cfg/sar_person_mix.yaml` (SARD + HERIDAL + NTUT4K)  
  - `cfg/sar_pos_only.yaml` (nur **SARD + HERIDAL**)

**Bekannte Warnung**  
- Roboflow Mixed (Boxes/Segments): Segmente werden verworfen, es werden **nur Bounding Boxes** verwendet.
...
- Train-Argumente: `--data`, `--weights`, `--epochs` (Default **20**), `--batch` (Default **6**), `--imgsz` (Default **800**)  
- Val-Only-Modus: `--val-only`, `--val-batch`, `--val-workers`, `--val-imgsz`, `--val-plots`  
- Optimierung: AdamW, `lr0=0.003`, `lrf=0.01`, `cos_lr=True`, `nbs=64`, `weight_decay=5e-4`, `warmup_epochs=1`, `patience=8`  
- Augmentierung:  
  `hsv_h=0.03, hsv_s=0.8, hsv_v=0.6, degrees=8, scale=0.85, translate=0.2, mosaic=0.15, mixup=0.0, copy_paste=0.1, close_mosaic=3`  
- Evaluation/NMS: `iou=0.5`, `max_det=150`, `classes=[0]`  
- No-label-Fallback (Val): Kennzahlen im **FP-only**-Modus

**Stabilität (Val/OOM-Mitigation)**  
...
- **Val (HERIDAL, light)**:  
  P ≈ **0.747**, R ≈ **0.628**, mAP50 ≈ **0.701**, mAP50-95 ≈ **0.295**  

- **Voll-Val (HERIDAL) nach Training**:  
  P ≈ **0.760–0.766**, R ≈ **0.656–0.664**, mAP50 ≈ **0.714–0.719**, mAP50-95 ≈ **0.284–0.293**  

- **Val (VisDrone)**:  
  P ≈ **0.510**, R ≈ **0.303**, mAP50 ≈ **0.315**, mAP50-95 ≈ **0.117**  

- **Val (NTUT4K, FP-only)**:  
  frühe Messung `avg_fp_per_image ≈ 6.58`

### 4.2 Kurz-Rewarm (SARD + HERIDAL), **4 Epochen** — 14.11.2025

- **Train-Val (HERIDAL)**:  
  P = **0.792**, R = **0.608**, mAP50 = **0.696**, mAP50-95 = **0.280**  

- **Externe Val (HERIDAL, 2276 Bilder)**:  
  P = **0.826**, R = **0.583**, mAP50 = **0.728**, mAP50-95 = **0.343**  

- **NTUT4K (FP-only, aktualisiert)**:  
  `avg_fp_per_image ≈ 0.0122` (10 Predictions / 819 Bilder)

---

## 5) Schwellen-Sweeps (Konfidenz-Varianten)
...
- Optionale „Slow-but-Sharp“-Val-Prüfung: `imgsz=896/960` (nur Val/Inference)
...
**`src/stage3_train_sar.py` (aktueller Stand)**  
- Train-Argumente: `--data`, `--weights`, `--epochs`, `--batch`, `--imgsz`  
- Val-Only: `--val-only`, `--val-batch`, `--val-workers`, `--val-imgsz`, `--val-plots`  
- Trainer: Optimierung/Aug/Eval wie oben; Ausgabe von JSON-Metriken + FP-only-Fallback
...
- Roboflow Mixed (Boxes/Segments): Segmente werden **ignoriert** → es werden nur Boxen genutzt.  
- NTUT4K: **keine Labels** → keine mAP, nur **FP-only**-Bewertung.  
- AMP/Assets-Downloads in Logs (z. B. „yolo11n.pt“, „Arial.ttf“) **ändern das Train-Backbone nicht**.  
- Einmalige NMS-Timeout-Warnung in einem frühen Lauf.
