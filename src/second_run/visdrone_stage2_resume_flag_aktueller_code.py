...
class CompleteSARTrainer:
    ...
    def stage1_visdrone_foundation(
        ...
        patience: int = 30,
        resume: bool = False,
        resume_from: Optional[Path] = None  # explicit resume checkpoint
    ) -> Path:
        """
        ...
        print(f"Epochs (target): {epochs} (patience: {patience})")
        ...
        # Resolve data_yaml to absolute path
        ...
        if resume and resume_from is not None:
            print(f"🔄 RESUME ENABLED: Continuing Stage 1 from checkpoint:")
            print(f"    {resume_from}\n")
            # Load checkpoint directly
            model = YOLO(str(resume_from))
            
            print("🏋️ Continuing Stage 1 training with Ultralytics resume=True")
            print("    → verwendet alle Hyperparameter & Scheduler aus dem Checkpoint\n")
            
            # Wichtig: keine Hyperparameter neu setzen, damit es wirklich
            # "weiterläuft, als wäre nichts passiert".
            results = model.train(
                resume=True,
            )
        ...
        # Determine best model path from the trainer's save_dir
        save_dir = Path(getattr(model, "trainer").save_dir) if hasattr(model, "trainer") else (self.models_dir / run_name)
        ...
        if metrics.get("mode", "GT") == "GT":
            ...
            print("\nℹ️  Stage 1 validation in FP-only mode (no GT labels found).")
    ...
    def run_full_pipeline(
        ...
        resume: bool = False,             # NEW: forwarded into Stage 1
        resume_from: Optional[Path] = None  # NEW: forwarded into Stage 1
    ):
        """
        ...
        if stage1_only:
            print(f"   Stage 1: Train/resume up to {stage1_epochs} epochs (THEN STOP)")
            print(f"   Stage 2: SKIP")
            print(f"   Stage 3: SKIP")
        else:
            print(f"   Stage 1: {'SKIP (using existing)' if skip_stage1 else f'Train/resume up to {stage1_epochs} epochs'}")
        ...
        if stage1_only:
            ...
            print(f"   python train_visdrone_baseline_and_other_stage2.py \\")
            print(f"       --skip-stage1 \\")
...
Examples:
  ...
  python train_visdrone_baseline_and_other_stage2.py
  
  # Full pipeline with Stage 3 (hard negative mining)
  python train_visdrone_baseline_and_other_stage2.py --enable-stage3
  
  # Skip Stage 1, use existing weights
  python train_visdrone_baseline_and_other_stage2.py --skip-stage1 --stage1-weights path/to/stage1_best.pt
  
  # Validation only
  python train_visdrone_baseline_and_other_stage2.py --val-only --weights path/to/model.pt
  
  # Custom epochs
  python train_visdrone_baseline_and_other_stage2.py --stage1-epochs 100 --stage2-epochs 30
        """
    )
    ...
    parser.add_argument("--resume-from", type=str, default=None,
                       help="Resume from specific checkpoint (e.g., path/to/last.pt)")
